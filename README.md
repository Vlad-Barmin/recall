...
Применимо к онбордингу сотрудников, корпоративному обучению, контролю знания регламентов и внутренней документации. Набор тем — параметр конфигурации, а не часть логики: в демо-наборе используются основы бэкенда (SQL, права доступа в базе, async/await, REST, Docker, Git).
...
```
Telegram Trigger → IF (фильтр) → Postgres (поиск вопроса) → Basic LLM Chain (сверка)
                                → Code (валидация) → Postgres (запись) → Postgres (статус) → Telegram
```
...
| `is_active` | boolean | участвует ли тема в выдаче |
...
| `missed` | text | что упущено или искажено |
| `explanation` | text | пояснение вердикта |
| `created_at` | timestamptz | время записи |

## Стек

- **n8n** (self-hosted, локальный запуск через npm)
...
Приём вебхуков от Telegram требует публичного адреса. Для локального запуска поднимается туннель:
...
Далее: импорт трёх воркфлоу из корня репозитория, создание креденшлов (Telegram, OpenRouter, Postgres), публикация воркфлоу. Расписание и вебхуки работают только у опубликованных воркфлоу — сохранение создаёт черновик.
